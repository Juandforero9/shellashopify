...
(function($) {
  ...
  class ProductPrice {
    ...
    insertPresentmentPrices($price, presentment_prices) {
      ...
      if (sale) {
        let formatedCompareAtPrice = Number(compare_at_price).toLocaleString('ja-JP', {
          style: 'currency',
      ...
      let formatedPrice = Number(price).toLocaleString('ja-JP', {
        style: 'currency',
        currency: Shopify.currency.active
      });
    ...
    load: function() {
      ...
      function toggle(e) {
        ...
        $element.find(_.selectors.button)
          ...
          .add($element.find('[' + _.settings.button + '="inner"]'))[$this.hasClass('open') ? 'addClass' : 'removeClass']('open');
      };
