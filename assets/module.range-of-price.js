...
(function($) {
  ...
  RangeOfPrice.prototype = $.extend({}, RangeOfPrice.prototype, {
    init: function() {
      ...
      this.dataMinFilterInputName = this.dom.$range.attr('data-min-filter-input-name');
      this.dataMaxFilterInputName = this.dom.$range.attr('data-max-filter-input-name');
        ...
        this.dom.$range.ionRangeSlider({
          ...
          prettify: function(data) {
            if (theme.MultiCurrency) {
              return theme.MultiCurrency.formatAndUpdate(data);
            } else {
              const placeholderRegex = /\{\{\s*(\w+)\s*\}\}/;

              if (theme.moneyFormat.match(placeholderRegex)[1] === 'amount') {
                return Shopify.formatMoney(data * 100, theme.moneyFormat).split('.')[0];
              }
          ...
          onFinish: function(data) {
            const $filterInputs = _.getMinMaxFilterInputs();

            if (!$filterInputs.min.length) {
              return;
            }

            $filterInputs.min.val(data.from !== data.min ? data.from : '');
            $filterInputs.max.val(data.to !== data.max ? data.to : '');

            $filterInputs.min.get(0).closest('form').dispatchEvent(new Event('input'));
          }
        });
      }
    },
    getMinMaxFilterInputs: function() {
      return {
        min: $(`[name="${this.dataMinFilterInputName}"]`),
        max: $(`[name="${this.dataMaxFilterInputName}"]`)
      }
    },
    destroy: function() {
    ...
    update: function() {
      if (this.dom.$range) {
        const api = this.dom.$range.data('ionRangeSlider'),
          $filterInputs = this.getMinMaxFilterInputs(),
          from = $filterInputs.min.val(),
          to = $filterInputs.max.val();

        api.update({
          from: from ? Number(from) : api.result.min,
          to: to ? Number(to) : api.result.max
        });
      }
    },
    reset: function() {
